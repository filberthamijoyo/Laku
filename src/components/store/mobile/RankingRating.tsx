...
export default function RankingRating({ top = 5, products }: Props) {
  const productsList: StoreProduct[] = products && products.length ? products : (mockStore && mockStore.products) || [];
  const ranked = productsList
    .slice()
    .sort((a, b) => {
      const ar = typeof a.rating === 'number' ? a.rating : parseFloat(String(a.rating || '0')) || 0;
      const br = typeof b.rating === 'number' ? b.rating : parseFloat(String(b.rating || '0')) || 0;
      return br - ar;
    })
    .slice(0, top);
